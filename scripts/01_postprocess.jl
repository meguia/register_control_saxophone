...
const LOG_OUT = joinpath(PROC_DIR, "postprocess_summary.txt")
...
function to_text(start_ts::DateTime, end_ts::DateTime, real_stats, model_stats)
    return """
...
function main()
    ...
    end_ts = now()
    report = to_text(start_ts, end_ts, real_stats, model_stats)
